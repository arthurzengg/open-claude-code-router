...
app.post('/v1/messages', express.json({ limit: '100mb' }), async (req, res) => {
  try {
    ...
    const cfg = getConfig();
    const decision = await decideRoute(body, sessions, {
      classify: cfg.classifier === 'heuristics-only' ? null : classify,
      haikuMaxTokens: cfg.haikuMaxTokens,
      models: cfg.models,
    });
    if (decision.reason !== 'passthrough') {
      body.model = decision.model;
      sanitizeBody(body, decision.label, cfg.limits, decision.model);
    }
    appendLog({
